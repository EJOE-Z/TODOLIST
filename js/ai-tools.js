        ...
        function: {
            ...
            parameters: {
                ...
                properties: {
                    ...
                    repeat: { type: 'string', enum: ['none', 'daily'], description: 'none=一次性，daily=每天重复' },
                    time_of_day: { type: 'string', description: '每日重复时的提醒时刻 HH:mm，如 18:30' }
                },
                required: ['title']
            }
        }
...
17. 设置提醒前若 time 已过期且非 daily，必须重新计算为未来时间
18. 用户说「记住XXX」时调用 remember_note；设置昵称/默认偏好时用 save_user_preference
19. 回复时适当参考用户偏好（如昵称、默认优先级、常用提醒时间、进行中任务的专注记录、时段排程偏好）
20. 用户说错过计划或专注结束时可建议「重排今日计划」
21. 创建任务时若标题匹配复习/考试类主题且未提供 subtasks，create_task 会自动拆分模板子任务
22. **课程作业/报告/实验类任务：必须先问清截止时间；子任务可选（用户说「无」可留空），标题含课程名或简称时自动关联课表**
23. create_task 若返回 needMoreInfo，说明信息不足，继续追问用户后再创建
