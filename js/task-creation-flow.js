...
class TaskCreationFlow {
    ...
    static parseTaskCommand(message) {
        ...
        if (/提醒$/.test(trimmed) && !/任务$/.test(trimmed)) {
            return null;
        }
    ...
    static extractTaskTitle(message) {
        ...
        if (/写日记|记日记|任务日记/.test(trimmed)) {
            return null;
        }

        if (TaskCreationFlow.isHomeworkTask(trimmed) && /太难|好难|好累|好烦|不想|头大|崩溃|救命|不会|写不出/.test(trimmed)) {
            return null;
        }
    ...
    static formatDeadlinePrompt(draft) {
        return [
            ...
            '',
            '请问这项任务的**截止时间**是什么时候？',
            '例如：`6月12日`、`下周五 18:00`、`下周三晚上19点`、`下周本课程上课前`',
            '若暂时没有，也可以回复 **无** 或 **暂不设置**。'
        ].join('\n');
    }
    ...
    static formatSubtasksPrompt(draft) {
        ...
        if (draft.suggestedSubtasks?.length) {
            lines.push('根据任务类型，我为您推荐了以下**子任务**：');
            draft.suggestedSubtasks.forEach((item, index) => {
                lines.push(`${index + 1}. ${item}`);
            });
            lines.push('');
            lines.push('回复 **确认** 采用推荐；也可以自行列出子任务；回复 **无** 则不添加。');
        } else {
            lines.push(
     ...
     * @returns {boolean}
     */
    static isDeadlineSkipped(text) {
        const trimmed = text.trim();
        return /^(跳过|不设|无|暂无|不知道|稍后|不用|不需要|暂不|none|no)$/i.test(trimmed);
    }

    /**
     * 初始化引导式创建草稿（「创建任务」多步流程，所有标题均询问截止时间与子任务）
     * @param {string} title
     * @returns {Object|null}
     */
    static initiateGuidedDraft(title) {
        const normalizedTitle = title.trim();
        if (!normalizedTitle) {
            return null;
        }
        if (typeof AIAssistant !== 'undefined' && AIAssistant.shouldInterruptTaskDraft(normalizedTitle)) {
            return null;
        }

        const course = TaskCreationFlow.matchCourseFromTitle(normalizedTitle);
        const suggestedSubtasks = typeof SubtaskTemplates !== 'undefined'
            ? SubtaskTemplates.suggest(normalizedTitle)
            : [];
    ...
    /**
     * 解析相对星期日期（下周三、本周五、周五 等）
     * @param {string} text
     * @returns {Date|null}
     */
    static resolveWeekdayDate(text) {
        ...
        let match = text.match(/下下周([一二三四五六日天])/);
        if (match) {
            weekOffset = 2;
            weekday = dayMap[match[1]];
        } else if ((match = text.match(/下周([一二三四五六日天])/))) {
            weekOffset = 1;
            weekday = dayMap[match[1]];
        } else if ((match = text.match(/(?:本周|这周)([一二三四五六日天])/))) {
            weekOffset = 0;
            weekday = dayMap[match[1]];
        } else if ((match = text.match(/(?:周|星期)([一二三四五六日天])/))) {
            weekOffset = null;
            weekday = dayMap[match[1]];
        }
        ...
        if (weekOffset !== null) {
            return TaskCreationFlow.getCourseDateInWeek(weekOffset, weekday);
        }
    ...
    /**
     * 从文本提取时分（支持 19:00、晚上19点、七点 等）
     * @param {string} text
     * @returns {{ hour: number, minute: number }|null}
     */
    static extractTimeFromText(text) {
        const trimmed = text.trim();
        const numPart = '[零一二两三四五六七八九十\\d]{1,2}';
        ...
        match = trimmed.match(new RegExp(`(?:上午|早上|中午|下午|晚上|夜间|傍晚)?(${numPart})点(\\d{1,2})分`));
        if (match) {
            const hour = TaskCreationFlow.parseChineseNumber(match[1]);
            if (hour !== null) {
                return {
        ...
        match = trimmed.match(new RegExp(`(?:上午|早上|中午|下午|晚上|夜间|傍晚)?(${numPart})点半`));
        if (match) {
            const hour = TaskCreationFlow.parseChineseNumber(match[1]);
            if (hour !== null) {
                return {
        ...
        match = trimmed.match(new RegExp(`(?:晚上|夜间|傍晚|午夜)?(${numPart})[点时](?:整)?`));
        if (match) {
            const hour = TaskCreationFlow.parseChineseNumber(match[1]);
            if (hour !== null) {
                return {
    ...
    static parseDeadline(text, draft = null) {
        ...
        const weekdayDate = TaskCreationFlow.resolveWeekdayDate(trimmed);
        if (weekdayDate) {
            const time = TaskCreationFlow.extractTimeFromText(trimmed);
            if (time) {
                weekdayDate.setHours(time.hour, time.minute, 0, 0);
            } else {
                weekdayDate.setHours(23, 59, 0, 0);
            }
            return weekdayDate.toISOString();
        }
    ...
    static advanceDraft(assistant, message) {
        ...
        if (draft.step === 'title') {
            if (typeof AIAssistant !== 'undefined' && AIAssistant.shouldInterruptTaskDraft(trimmed)) {
                assistant.taskDraft = null;
                return null;
            }
            const nextDraft = TaskCreationFlow.initiateGuidedDraft(trimmed);
            if (!nextDraft) {
                if (typeof AIAssistant !== 'undefined' && AIAssistant.shouldInterruptTaskDraft(trimmed)) {
                    assistant.taskDraft = null;
                    return null;
                }
                return '请告诉我**任务标题**是什么？\n\n请勿输入快捷指令（如「创建任务」「开始专注」），请直接说任务名称。';
            }
        ...
        if (draft.step === 'deadline') {
            ...
            if (TaskCreationFlow.isDeadlineSkipped(trimmed)) {
                draft.deadline = null;
                draft.step = 'subtasks';
            ...
            if (!deadline) {
                return '我没有识别到有效的截止时间，请再说一次，例如：`6月12日`、`下周五 18:00`、`下周三晚上19点`；若暂不设置可回复 **无**。';
            }
        ...
        if (draft.step === 'subtasks') {
            const suggested = draft.suggestedSubtasks || [];
            if (/^(确认|好的|可以|采用|就用这些|默认|ok|yes)$/i.test(trimmed) && suggested.length) {
                draft.subtasks = [...suggested];
            } else if (TaskCreationFlow.isSubtasksSkipped(trimmed)) {
                draft.subtasks = [];
            } else {
