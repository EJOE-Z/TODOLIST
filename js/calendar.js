class CalendarManager {
    ...
    renderCalendar() {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            html += `
                <div
                    class="calendar-day-cell${isToday ? ' calendar-day-today' : ''}${isSelected && !isToday ? ' calendar-day-selected' : ''}${hasTasks ? ' calendar-day-has-task' : ''}${hasReminders ? ' calendar-day-has-reminder' : ''}"
                    data-date="${dateKey}"
    ...
    showDayTasks(date) {
        ...
        if (dayReminders.length > 0) {
            ...
            dayReminders.forEach(reminder => {
                ...
                const repeatLabel = reminder.repeat === 'daily' ? ' · 每日' : '';
                html += `
