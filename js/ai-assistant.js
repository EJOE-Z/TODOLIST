...
class AIAssistant {
    ...
    setupEventListeners() {
        ...
        document.getElementById('chat-history')?.addEventListener('click', (e) => {
            ...
            if (!btn || this.isProcessing) return;
    ...
    async checkBackendStatus() {
        ...
        try {
            ...
            if (statusEl) {
                statusEl.textContent = '离线 · 请运行 backend/app.py';
            }
        }
    }
    ...
    async processMessage(message) {
        ...
        try {
            ...
            if (deleteReminderMatch && !deleteReminderMatch.confirmed) {
                this.hideTypingIndicator();
                this.addMessage('assistant', `确定删除提醒「${deleteReminderMatch.query}」吗？`, {
                    confirm: {
            ...
            if (postponeMatch) {
                const title = postponeMatch[1].trim();
                UserMemory.postponePlanTask(title);
                const plan = SecretaryEngine.generateDailyPlan();
                const content = `⏭️ 已将「${title}」推迟 30 分钟，更新后的计划：\n\n${SecretaryEngine.formatDailyPlanMarkdown(plan)}`;
                this.hideTypingIndicator();
            ...
            if (typeof TaskCreationFlow !== 'undefined') {
                ...
                if (taskCmd) {
                    ...
                    if (taskCmd.action === 'delete') {
                        ...
                        result = ActionExecutor.completeTask({ task_query: taskCmd.query });
                        reply = result.success ? `✅ ${result.message}` : `❌ ${result.error}`;
                    }
                    this.apiMessages.push({ role: 'user', content: message });
    ...
    static parseDeleteReminderQuery(message) {
        const confirm = message.match(/^确认删除提醒[：:\s]*(.+)$/i);
        if (confirm) return { query: confirm[1].trim(), confirmed: true };
        const del = message.match(/^删除提醒[：:\s]*(.+)$/i);
        if (del) return { query: del[1].trim(), confirmed: false };
        return null;
    }
    ...
    async fallbackProcess(message) {
        ...
        if (typeof TaskCreationFlow !== 'undefined') {
            ...
            if (taskCmd) {
                ...
                const result = taskCmd.action === 'delete'
                    ...
                    : ActionExecutor.completeTask({ task_query: taskCmd.query });
                return result.success ? `✅ ${result.message}` : `❌ ${result.error}`;
            }
        }
        ...
        if (/开始专注|专注模式|番茄钟/i.test(message)) {
