...
class PlanRescheduleEngine {
    ...
    /**
     * 重算今日计划（清除推迟与错过标记后全量重排）
     * @returns {Object}
     */
    static rescheduleToday() {
        UserMemory.clearPlanPostpones();
     ...
     * 解析推迟计划命令
     * @param {string} raw
     * @returns {{ taskTitle: string, subtaskTitle: string|null }}
     */
    static parsePostponeCommand(raw) {
        const text = String(raw || '').trim();
        if (text.includes('|')) {
            const [taskTitle, subtaskTitle] = text.split('|').map(s => s.trim());
            return { taskTitle, subtaskTitle: subtaskTitle || null };
        }
        return { taskTitle: text, subtaskTitle: null };
    }

    /**
     * 将今日计划快照中的某槽推迟若干分钟（仅移动匹配槽，其余不变）
     * @param {string} taskTitle
     * @param {string|null} [subtaskTitle]
     * @param {number} [deltaMinutes]
     * @returns {Object|null}
     */
    static postponeSlotMinutes(taskTitle, subtaskTitle, deltaMinutes = 30) {
        const plan = PlanRescheduleEngine.getTodayPlan();
        if (!plan) return null;
        ...
        const slot = plan.slots.find(s =>
            s.status === 'pending'
            && s.taskTitle === taskTitle
            && (subtaskTitle ? s.subtaskTitle === subtaskTitle : true)
        );
        if (!slot) return null;

        slot.startMinutes += deltaMinutes;
        slot.endMinutes += deltaMinutes;
        ...
        for (let i = 0; i < pending.length - 1; i++) {
            if (pending[i].endMinutes > pending[i + 1].startMinutes) {
                const shift = pending[i].endMinutes - pending[i + 1].startMinutes;
                pending[i + 1].startMinutes += shift;
                pending[i + 1].endMinutes += shift;
            }
        }
