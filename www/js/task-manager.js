/**
 * 任务完成耗时（创建 → 完成），与专注时长 focusTime 分离
 */
const TaskDuration = {
    /**
    ...
    /**
     * 根据创建/完成时间写入 duration
     * @param {Object} task
     */
    applyToCompletedTask(task) {
        ...
        task.duration = TaskDuration.formatMilliseconds(Math.max(0, diff));
    },
     ...
     * @returns {number}
     */
    getCompletionMinutes(task) {
        if (!task.completed) {
            return 0;
        }
        if (task.duration) {
            return TaskDuration.parseMinutes(task.duration);
        }
...
class TaskManager {
    ...
    toggleSubtaskCompletion(taskId, subtaskId) {
        ...
        if (task && task.subtasks) {
            ...
            if (subtask) {
                ...
                const allDone = subtasks.length > 0 && subtasks.every(item => item.completed);
                if (allDone) {
                    task.completed = true;
                    task.completedAt = new Date().toISOString();
                    TaskDuration.applyToCompletedTask(task);
    ...
    openDiaryModal(taskId) {
        ...
        if (task) {
            ...
            if (historyList) {
                const diaries = [...(task.diary || [])].reverse();
                if (diaries.length) {
                    historyList.classList.remove('hidden');
                    historyList.innerHTML = `
                        <p class="diary-history-label">历史记录（${diaries.length} 篇）</p>
                        ${diaries.map((entry) => `
                            <div class="diary-history-item">
                                <span class="diary-history-date">${entry.date || ''}</span>
                                <p class="diary-history-text">${DiaryView.escapeHtml(entry.content).replace(/\n/g, '<br>')}</p>
                            </div>
                        `).join('')}
                    `;
                } else {
    ...
    saveDiary() {
        ...
        if (task) {
            ...
            this.openDiaryModal(this.currentTaskId);
        }
    }
    ...
    updateProgress() {
        ...
        const completedTodayList = this.tasks.filter(t =>
        ...
        if (dueToday.length > 0) {
            completedCount = dueToday.filter(t => t.completed).length;
            totalToday = dueToday.length;
            hint = '今日截止';
        } else {
            completedCount = completedTodayList.length;
            totalToday = pending.length + completedCount;
            hint = pending.length > 0 ? '今日可推进' : '今日';
        }
        ...
        if (progressText) {
            progressText.textContent = totalToday > 0 ? `${completedCount}/${totalToday}` : '0/0';
        }
        if (progressHint) {
