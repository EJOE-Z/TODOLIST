...
class SecretaryEngine {
    ...
    static generateDailyPlan() {
        ...
        /** @type {Array<Object>} */
        const remainingUnits = workUnits.filter(u => !UserMemory.isPlanSlotPostponed(u.taskTitle, u.subtaskTitle));
    ...
    /**
     * 用今日计划快照中的槽位覆盖排程里的专注块（推迟后保持其余任务不变）
     * @param {Object} [basePlan]
     * @returns {Object}
     */
    static mergeTodaySlotsIntoPlan(basePlan) {
        if (typeof PlanRescheduleEngine === 'undefined') {
            return basePlan || SecretaryEngine.generateDailyPlan();
        }

        const todayPlan = PlanRescheduleEngine.getTodayPlan();
        basePlan = basePlan || SecretaryEngine.generateDailyPlan();
        if (!todayPlan?.slots?.length) return basePlan;

        const suggestionBlocks = todayPlan.slots
            .filter(s => s.status === 'pending')
            .sort((a, b) => a.startMinutes - b.startMinutes)
            .map(s => {
                const duration = (s.endMinutes || s.startMinutes + 25) - s.startMinutes;
                return {
                    type: 'suggestion',
                    time: `${SecretaryEngine.formatMinutes(s.startMinutes)}-${SecretaryEngine.formatMinutes(s.endMinutes || s.startMinutes + duration)}`,
                    startMinutes: s.startMinutes,
                    endMinutes: s.endMinutes || s.startMinutes + duration,
                    label: s.label,
                    taskTitle: s.taskTitle,
                    subtaskTitle: s.subtaskTitle,
                    duration,
                    detail: `专注 ${duration} 分钟`
                };
            });

        const otherBlocks = basePlan.blocks.filter(b => b.type !== 'suggestion');
        const blocks = [...otherBlocks, ...suggestionBlocks]
            .sort((a, b) => (a.startMinutes ?? 9999) - (b.startMinutes ?? 9999));
        const courseCount = blocks.filter(b => b.type === 'course').length;

        return {
            ...basePlan,
            blocks,
            summary: SecretaryEngine.buildPlanSummary(blocks, basePlan.unscheduledTasks || [], courseCount)
        };
    }
